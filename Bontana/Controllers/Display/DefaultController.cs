        ...
        public PartialViewResult productHomes()
        {
            ...
            var groupProduct = db.TblGroupProduct.Where(p => p.Active == true && p.Priority == true  ).OrderBy(p => p.Ord).ToList();
                ...
                for (int i = 0; i < groupProduct.Count; i++)
                {
                    ...
                    result.Append("<div class=\"Floor\" style=\"background:" + tblconfig.Color + "\">");
                    result.Append("<div class=\"Content_Floor\">");
                    result.Append("<div class=\"LeftFloor\" style=\"background:" + tblconfig.Color + "\">");
                    result.Append("<div class=\"Leftfloor1\"><span>"+ manufactures.Name + " </span></div>");
                    result.Append("<div class=\"circle\" style=\"color:" + tblconfig.Color + "\">"+(i+1)+"</div>");
                    result.Append("</div>");
                    result.Append("<div class=\"Center_Floor\">");
                    result.Append("<h2><a href=\"/" + groupProduct[i].Tag + ".html\" title=\"" + groupProduct[i].Name + "\">" + groupProduct[i].Name + "</a></h2>");
                    result.Append("</div>");
                    ...
                    {
                        result.Append("<a href=\"/" + groupProductChild[j].Tag + ".html\" title=\"" + groupProductChild[j].Name + "\">" + groupProductChild[j].Name + "</a>");
                    }
        ...
        public PartialViewResult newsVideoHomes()
        {
            var tblconfig = db.TblConfig.FirstOrDefault();
                ...
                for(int i=0;i<listNews.Count;i++)
                {
                    ...
                    result.Append("<a href=\"/tin-tuc/" + listNews[i].Tag+ "\" title=\"" + listNews[i].Name + "\"><img src=\"" + listNews[i].Images + "\" alt=\"\" /></a>");
                    result.Append("<a class=\"name\" href=\"/tin-tuc/" + listNews[i].Tag + "\" title=\"" + listNews[i].Name + "\">" + listNews[i].Name + "</a>");
