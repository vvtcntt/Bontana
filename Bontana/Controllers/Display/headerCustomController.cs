    ...
    public class headerCustomController : Controller
    {
        ...
        public PartialViewResult partialHeader()
        {
                ...
                for(int i=0;i<listGroupProduct.Count;i++)
                {
                    ...
                    resultMobile.Append("<a href=\"/" + listGroupProduct[i].Tag + ".html\" title=\"" + listGroupProduct[i].Name + "\">" + listGroupProduct[i].Name + "</a>");                
                 


                    result.Append("<li class=\"li1\">");
                    result.Append("<a href=\"/"+listGroupProduct[i].Tag+".html\" title=\""+listGroupProduct[i].Name+"\">");
                    ...
                    if(listGroupChild.Count>0)
                    {
                        ...
                        for(int j=0;j<listGroupChild.Count;j++)
                        {
                            ...
                            result.Append("<a href=\"/"+listGroupChild[j].Tag+ ".html\" title=\"" + listGroupChild[j].Name + "\">");
                            result.Append("<img src=\"" + listGroupChild[j].Images + "\" title=\"" + listGroupChild[j].Name + "\" />");
                            result.Append("</a>");
                            result.Append("<a href=\"/" + listGroupChild[j].Tag + ".html\" class=\"nameMenu\" title=\"" + listGroupChild[j].Name + "\">" + listGroupChild[j].Name + "</a>");
                            result.Append("</li>");
                            resultMobile.Append("<li><a href=\"/" + listGroupChild[j].Tag + ".html\" title=\"" + listGroupChild[j].Name + "\">" + listGroupChild[j].Name + "</a></li> ");
